...
% A test:
centre_px = [2430 2187]; % X Y 
star_px = [1389 2893]; % X Y
roi_size = [150,150];
roi = [star_px(2)-floor(roi_size(2)/2),star_px(2)+ceil(roi_size(2)/2); %Y
            star_px(1)-floor(roi_size(1)/2),star_px(1)+ceil(roi_size(1)/2)]; %X
...
sections = {};
sections{1} = im2double(imgs{1}(roi(1,1):roi(1,2),roi(2,1):roi(2,2),:));
sections{2} = im2double(imgs{2}(roi_offset(1,1):roi_offset(1,2),roi_offset(2,1):roi_offset(2,2),:));
cli_header('Snip!');

sub_corr_1 = normxcorr2(rgb2gray(sections{1}),rgb2gray(sections{1}));
sub_corr_2 = normxcorr2(rgb2gray(sections{2}),rgb2gray(sections{2}));
cross_corr = normxcorr2(rgb2gray(sections{1}),rgb2gray(sections{2}));

cli_header(1,'Making histograms!');
%
    num_bins = 2^8;

    bin_edges = linspace(0,max(sections{2},[],'all'),num_bins+1);
    ...
    for i=1:2
        counts.red(i,:) = histcounts(sections{i}(:,:,1),bin_edges);
        counts.blue(i,:) = histcounts(sections{i}(:,:,2),bin_edges);
        counts.green(i,:) = histcounts(sections{i}(:,:,3),bin_edges);
    end


fig_h = 2;
fig_w = 3;
stfig('Show sub images');
clf;
    subplot(fig_h,fig_w,1)
    hold on
    imagesc(sections{1})
...
clf;
    ...
    subplot(fig_h,fig_w,1)
    imagesc(sub_corr_1(ceil(roi_size(1)/2):3*floor(roi_size(1)/2),ceil(roi_size(1)/2):3*floor(roi_size(1)/2)))
    
    subplot(fig_h,fig_w,2)
    imagesc(sub_corr_2(ceil(roi_size(1)/2):3*floor(roi_size(1)/2),ceil(roi_size(1)/2):3*floor(roi_size(1)/2)))
    
    subplot(fig_h,fig_w,3)
    imagesc(cross_corr(ceil(roi_size(1)/2):3*floor(roi_size(1)/2),ceil(roi_size(1)/2):3*floor(roi_size(1)/2)))
