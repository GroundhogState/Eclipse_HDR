...
def channel_stats(img):
    chan_means = [np.mean(np.ndarray.flatten(img[:,:,chan])) for chan in np.r_[0:img.shape[-1]]]
    chan_stds = [np.std(np.ndarray.flatten(img[:,:,chan])) for chan in np.r_[0:img.shape[-1]]]
...
def exposure_scaling(img_paths,rgn,all_exp,sigma=0):
    ...
    for im_idx in np.r_[0:num_imgs]:
        ...
        [mn[im_idx],st[im_idx]] = channel_stats(test_img[rgn[0][0]:rgn[0][1],rgn[1][0]:rgn[1][1]])
...
def channel_registration(data_root,img_array,grey_imgs,frame_size,sigma=5,mask_level=0.4,
    verbose=0,subsampwidth=500,img_names=[]):
    ...
    for im_idx in np.r_[0:num_imgs]:
        ...
        if im_idx == 0:
            ...
            sum_out += new_out
            r_new = standard_framing(img_array[im_idx][:,:,0],frame_size,shift=shifts)
            g_new = standard_framing(img_array[im_idx][:,:,1],frame_size,shift=shifts)
            b_new = standard_framing(img_array[im_idx][:,:,2],frame_size,shift=shifts)
...
def vips_channel_registration(data_root,img_paths,frame_size,sigma=5,mask_level=0.4,
    verbose=0,subsampwidth=500,img_names=[]):
    ...
    for im_idx in np.r_[0:num_imgs]:
        ...
        if im_idx == 0:
            ...
            if verbose>0:
                ...
                print(' Shifts:    (%u,%u)'%(shifts[0],shifts[1]))
                print(' sum shape: (%u,%u)'%sum_out.shape)
            new_out = standard_framing(new_img,frame_size,verbose=verbose-1,shift=shifts)
        ...
        vi = pyvips.Image.new_from_memory(linear.data, width, height, bands,
                                          dtype_to_format[str(this_rgb.dtype)])
        vi.write_to_file(fname)
    rgb_sum = np.int_(np.r_[[r_sum],[g_sum],[b_sum]])
    io.imsave(data_root+"N_profile"+'.jpg',n_sum)
    return rgb_sum,n_sum