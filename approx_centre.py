...
for im_idx in np.r_[0:num_imgs]:
    ...
    opt_res = scp.optimize.minimize(
        ...
        x0,
        method="nelder-mead",
        options={"xtol": 1e-8, "disp": opt_verbose},
    )
    ...
    vi = pyvips.Image.new_from_memory(
        linear.data, width, height, bands, dtype_to_format[str(img_crop.dtype)]
    )
    ...
    if visual_out:
        ...
        plt.subplot(1, 3, 1)
        plt.imshow(
            edges[my[0] : my[1], mx[0] : mx[1]], extent=[mx[0], mx[1], my[1], my[0]]
        )
        plt.subplot(1, 3, 2)
        plt.imshow(
            this_img[my[0] : my[1], mx[0] : mx[1]], extent=[mx[0], mx[1], my[1], my[0]]
        )
